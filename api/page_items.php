...
// Initialize empty array for page items
$page_items = [];
...
$sql = "SELECT page_items.*, image_items.item_name AS image_item_name, subcategory_items.item_name AS subcategory_item_name
        FROM page_items 
        LEFT JOIN image_items ON page_items.image_id = image_items.image_item_id
        LEFT JOIN subcategory_items ON page_items.subcategory_id = subcategory_items.subcategory_item_id";

$conditions = ["page_items.page_item_id != 999"]; // Exclude person_item_id = 999

if ($category > 0) {
    $conditions[] = "page_items.category_id = " . $category;
}
if ($subcategory > 0) {
    $conditions[] = "page_items.subcategory_id = " . $subcategory;
}
...
if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        $row['page_item_id'] = intval($row['page_item_id']);
...
echo json_encode(['results' => $page_items], JSON_UNESCAPED_UNICODE);
