...
// Initialize empty array for person items
$person_items = [];
...
$sql = "SELECT person_items.*, image_items.item_name AS image_item_name, subcategory_items.item_name AS subcategory_item_name
        FROM person_items 
        LEFT JOIN image_items ON person_items.image_id = image_items.image_item_id
        LEFT JOIN subcategory_items ON person_items.subcategory_id = subcategory_items.subcategory_item_id";

$conditions = ["person_items.person_item_id != 1"]; // Exclude person_item_id = 1

if ($category > 0) {
    $conditions[] = "person_items.category_id = " . $category;
}
if ($subcategory > 0) {
    $conditions[] = "person_items.subcategory_id = " . $subcategory;
}
...
if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        $row['person_item_id'] = intval($row['person_item_id']);
...
echo json_encode(['results' => $person_items], JSON_UNESCAPED_UNICODE);
